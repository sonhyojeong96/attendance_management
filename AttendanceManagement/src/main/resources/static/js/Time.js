...
function attTime() {
	...
	attClock.innerText = msg; //innerText : 불필요한 공백을 제거하고 텍스트로 반환

	$.ajax({
		url : "time.att",
		data : "clock=" + attClock.textContent, // textContent : 모든 텍스트를 그대로 가져옴
		type : "POST",
			...
			.datepicker(
					{
						...
						dayNames : [ '日曜日', '月曜日', '火曜日', '水曜日', '木曜日', '金曜日',
								'土曜日' ],
						altField : '#start',
						onSelect : function(dateText) {　//onSelect는 Datepicker에서 날짜가 선택되었을떄 발생하는 이벤트
							$
									.ajax({
										type : "GET",
										url : "time.bring",
										data : "dateText=" + dateText, //선택된 날짜텍스트
										async : false,
										success : function() {
											var url = "/timePopup";
											var name = "popup test";
